...
function pushHistory() {
    if (!window.EditorHistory) return;

    window.EditorHistory.push(createState());
...
function undo() {
    ...
    const previousState = window.EditorHistory.undo(currentState);
...
function redo() {
    ...
    const currentState = createState();
    const nextState = window.EditorHistory.redo(currentState);
...
function updateHistoryButtons() {
    ...
    if (undoButton) {
        undoButton.disabled = !window.EditorHistory.canUndo();
    }

    if (redoButton) {
        redoButton.disabled = !window.EditorHistory.canRedo();
    }
}
